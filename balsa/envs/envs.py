...
class Workload(object):
    ...
    def FilterQueries(self, query_dir, query_glob, test_query_glob):
        ...
        print(f"Found {len(all_sql_set)} SQL files in all query nodes.")
        print(f"Found {len(all_sql_set_new)} SQL files in new query nodes.")
        ...
        print("--------")
        print(f'Filtered to {len(train_nodes_new)} training queries and {len(test_nodes_new)} test queries.')
        print("--------")
    ...
    def FilterQueries_sep_dirs(self, query_dir, query_glob, test_query_glob, test_query_dir = None):
        all_sql_set_new = self._get_sql_set(query_dir, query_glob)
        if not test_query_dir:
            print("No separate test_query_dir provided, using query_dir for test queries.")
            test_sql_set_new = self._get_sql_set(query_dir, test_query_glob)
            assert test_sql_set_new.issubset(all_sql_set_new), (test_sql_set_new,
                                                            all_sql_set_new)
        else:
            print("Using separate test_query_dir for test queries.")
            test_sql_set_new = self._get_sql_set(test_query_dir, test_query_glob)
            print(f"Found {len(test_sql_set_new)} test queries in separate test directory.")
        ...
        test_nodes_new = [
            n for n in self.test_nodes if n.info['path'] in test_sql_set_new
        ]
        assert len(train_nodes_new) > 0
        print("--------")
        print(f'Filtered to {len(train_nodes_new)} training queries and {len(test_nodes_new)} test queries.')
...
class JoinOrderBenchmark(Workload):
    ...
    def _LoadQueries_sep_dirs(self):
        ...
        if p.test_query_dir is not None:
            ...
            # print(f"Searching test_query_dir: {p.test_query_dir} with pattern: {test_glob_pattern}")
            ...
            # print(f"Found {len(test_sql_files_from_test_dir)} SQL files in test_query_dir.")
        else:
            # print("No separate test_query_dir provided, using query_dir for test queries.")
        ...
        # print(f"Found {len(all_sql_files_from_primary_dir)} SQL files in primary query_dir.")
        # print(f"Found {len(test_sql_files_from_test_dir)} SQL files in test_query_dir.")
        # exit(0)
        ...
        if p.workload_order_file and os.path.isfile(p.workload_order_file):
            ...
            test_paths = set(unified_sql_file_paths) - train_paths
